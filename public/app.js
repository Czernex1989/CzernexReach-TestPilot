...
function renderLoading() {
  ...
  resultEl.innerHTML = '<p class="loading">Uruchamianie testu…</p>';
}
...
function renderResult(data) {
  ...
  const issuesHtml = issues.length
    ? `<div class="result-issues"><h3>Znalezione problemy (${issues.length})</h3><ul>${issues
        .map((i) => `<li>${escapeHtml(i)}</li>`)
        .join('')}</ul></div>`
    : '';

  const screenshotHtml = data.screenshot
    ? `<div class="result-screenshot"><img src="/screenshots/${encodeURIComponent(data.screenshot)}" alt="Screenshot błędu" /></div>`
    : '';

  resultEl.hidden = false;
  resultEl.innerHTML = `
    <span class="result-status ${data.status}">${data.status}</span>
...
form.addEventListener('submit', async (event) => {
  ...
  try {
    ...
    if (!response.ok) {
      renderError(data.error || 'Test nie powiódł się.');
      return;
    }
