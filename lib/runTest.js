...
async function runMobileCheck(browser, url) {
  ...
  try {
    ...
    if (!loaded || httpStatus === null || httpStatus >= 400) {
      status = 'FAIL';
    } else {
      hasViewportMeta = await page
        .$eval('meta[name="viewport"]', (el) => /width\s*=\s*device-width/i.test(el.getAttribute('content') || ''))
        .catch(() => false);
      hasHorizontalOverflow = await page.evaluate(
        () => document.documentElement.scrollWidth > document.documentElement.clientWidth + 1
      );

      if (!hasViewportMeta || hasHorizontalOverflow) {
        status = 'WARNING';
      }
    }
    ...
    screenshotFile = `mobile-${Date.now()}.png`;
    try {
  ...
  return { status, httpStatus, hasViewportMeta, hasHorizontalOverflow, screenshot: screenshotFile, error };
}
...
async function runTest(rawUrl) {
  ...
  try {
    ...
    const mobile = await runMobileCheck(browser, url);

    const failed = Boolean(navigationError) || httpStatus === null || httpStatus >= 400 || mobile.status === 'FAIL';
    const hasWarnings = consoleErrors.length > 0 || brokenLinks.length > 0 || mobile.status === 'WARNING';
